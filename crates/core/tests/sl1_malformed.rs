...
fn an_empty_file_is_not_identified_as_anything() {
    ...
    assert!(
        registry::identify(&path).is_err(),
        "an empty file must not identify"
    );
}
...
fn random_bytes_named_sl1_are_rejected() {
    ...
    assert!(
        registry::identify(&path).is_err(),
        "contents decide, not the name (§11)"
    );
}
...
fn a_layer_that_is_not_a_png_fails_to_decode_cleanly() {
    let (_d, path) = archive(
        "badpng.sl1",
        &[
            ("config.ini", GOOD_CONFIG),
            ("a00000.png", b"not a png at all"),
        ],
    );
...
fn a_declared_layer_count_that_disagrees_produces_a_warning_not_a_failure() {
    ...
    assert!(
        warnings[0].contains("100") && warnings[0].contains("1"),
        "got: {warnings:?}"
    );
}
